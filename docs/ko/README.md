...
<p align="center">
  ...
  <br/>
  <sub>15명의 신이 논쟁하고, 도전하고, 검증한다 — 소프트웨어가 가정에 기대어 출시되지 않도록.</sub>
</p>
...
Olympus는 Claude Code용 **하네스 엔지니어링 플러그인**이다. 15명의 에이전트 — 각각 고유한 역할과 엄격한 권한을 가진 그리스 신 — 가 구조화된 적대적 협업을 통해 요구사항을 정제하고, 다양한 관점에서 분석하고, 소프트웨어를 구현하고 평가한다.
...
| **Pantheon** | Consensus | ≥ 66% | 다관점 분석이 악마의 변호인을 살아남음 |
...
| **Nemesis** | PR 리뷰 종합자 — 발견 사항 중복제거 및 판결 | opus | 읽기 전용 |

### 위임 패턴

15명 중 10명의 에이전트가 **읽기 전용**이다. 파일을 쓸 수 없다. 대신:
