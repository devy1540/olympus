...
**상황**: Oracle로 스펙을 뽑았고, 구현 전에 스펙을 스트레스 테스트하고 싶다.

```
나:  /olympus:pantheon
```

Pantheon이 이전 Oracle 실행의 spec.md를 읽고 분석을 시작한다.

**진행 과정:**

1. **Hermes**가 코드베이스 탐색 — 기존 모듈, 의존성, 테스트 커버리지 파악
2. **Helios**가 복잡도를 평가하고 직교 관점을 생성:
   - 코드 품질 (결합도, 응집도, 기존 패턴)
   - 보안 (공격 표면, 인증 경계, 데이터 흐름)
   - 아키텍처 (새 기능의 위치, 통합 지점)
3. **Ares + Poseidon + Zeus**가 병렬 분석 — 각각 `file:line` 증거와 함께 소견 작성
4. **Eris** (악마의 변호인)가 모든 결론에 도전:
   "Ares는 서비스 레이어가 깨끗하다고 하지만, `OrderService`의 `src/order/service.ts:142`에 의존성이 6개 — 그건 깨끗한 게 아니다."
5. 합의: 분석가 투표, 오케스트레이터 집계 → 75% (실무적) — 통과

결과물: `.olympus/{id}/analysis.md` — 증거가 포함된 통합 분석

**Pantheon을 쓸 때:**
- Oracle 후, 구현 전 — 스펙을 스트레스 테스트
- 리팩토링 대상 모듈을 검토할 때
- 코드에 대해 여러 전문가 시각이 필요할 때

---

## 예시 4: 기술적 의사결정

**상황**: 결제 모듈의 기술 부채가 커지고 있다. 지금 리팩토링할지, 어댑터로 감싸서 나중에 할지 결정해야 한다.
...
1. **프레이밍**: 오케스트레이터가 질문 구조화 — 누가 영향 받는지, 제약은 뭔지, 기준은 뭔지
2. **위원회** (3인 토론):
   - Zeus (아키텍트): "어댑터가 리스크가 낮고, 릴리스 일정에 영향 안 준다"
   - Ares (엔지니어): "어댑터는 간접 레이어를 추가하는데, 아무도 나중에 안 걷어낸다"
   - UX 비평가: "사용자는 내부 구조 신경 안 쓴다 — 기능부터 출시해라"
3. **토론** (최대 3라운드): 각 역할이 상대 논점에 반론
4. **Eris** 도전:
   "'나중에'가 다음 분기라고 다들 가정하는데. 이 팀이 마지막으로 감싼 코드를 실제로 돌아가서 고친 적이 언제인지 보여줘."
5. **결정**: 다수 의견의 근거와 반대 의견이 기록된 구조화된 판결

**Agora를 쓸 때:**
- "지금 할까 나중에 할까" 결정
- 두 구현 방식 중 선택
- 감이 아니라 구조화된 논증이 필요할 때
